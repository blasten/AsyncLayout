...
export default class LayoutVertical extends HTMLElement {
  ...
  set props(v) {
    let oldProps = this._props, newProps = Object.assign({}, oldProps, v);
  ...
  async _refresh(scroll) {
    ...
    if (scroll) {
      this._scrollTo(this.props.sectionIndex, this.props.rowIndex);
    }
    await recycler.fillBuffer();
  }
  ...
  _copyMeta(meta) {
    let intervals = this._intervals,
      ...
      rowIdx = bottom
        ? tidx - intervals[secIdx][0]
        : tidx - intervals[secIdx][0] - 1,
  ...
  _scrollTo(secIdx, rowIdx = UNKNOWN_IDX) {
    ...
    setScrollTop(
      se,
      storage && storage.y > 0 ? storage.y : idx * this._medianHeight
    );
  }
